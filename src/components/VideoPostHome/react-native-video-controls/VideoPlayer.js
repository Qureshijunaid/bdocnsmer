...
const VIDEO_WIDTH = Dimensions.get("window").width - 30;
...
export default class VideoPlayer extends Component {
	...
	constructor(props) {
		...
		this.state = {
			...
			showSeker: false,
			customPlay: false,
			playedTime: (this.props.playedTime / this.props.duration) * 100,
			totalTime: (100 - ((this.props.playedTime / this.props.duration) * 100)),
			showSpeaker: false,
			videoHeight: constants.vh(180),
		};
		 ...
		 */
		this.player = {
			controlTimeoutDelay: this.props.controlTimeout || 1500,
	...
	_onLoad(data = {}) {
		let state = this.state;
		this._calculateVideoHeight(data)
	...
	_calculateVideoHeight(data) {
		var videoHeight = VIDEO_WIDTH
		var tempHeight = VIDEO_WIDTH
		switch (this.props.aspectType) {
			case "11":
				videoHeight = VIDEO_WIDTH
				break;
			case "43":
				tempHeight = VIDEO_WIDTH * 1.33
				videoHeight = tempHeight < HEIGHT ? tempHeight : HEIGHT
				break;

			case "34":
				tempHeight = VIDEO_WIDTH * 0.75
				videoHeight = tempHeight < HEIGHT ? tempHeight : HEIGHT
				break;

			case "169":
				tempHeight = VIDEO_WIDTH * 0.53
				videoHeight = tempHeight < HEIGHT ? tempHeight : HEIGHT
				break;

			case "916":
				tempHeight = VIDEO_WIDTH * 1.77
				videoHeight = tempHeight < HEIGHT ? tempHeight : HEIGHT
				break;

			default:
				const { width, height } = data.naturalSize;
				const heightScaled = height / width * VIDEO_WIDTH
				videoHeight = heightScaled
		}
		let state = this.state;
	...
	_togglePlayPause() {
		...
		if (this.state.paused === false) {
			this.setState({ showSeker: true, customPlay: true }, () => { }
			)
		}
		else {
			this.setState({ showSeker: false, customPlay: false }, () => { }
			)
		}
	...
	handleMuteUnmute = () => {
		this.setState({
			...
			this.props.handleVolumeMuteUnmute(this.state.muted)
		})
	...
	render() {
		return (
			<TouchableWithoutFeedback
				//onPress={this.events.onScreenTouch, this.handleMuteUnmute}
				...
				<View style={[styles.player.container, this.styles.containerStyle, {
					...
					<LinearGradient
						...
						colors={[
							...
							"rgba(0,0,0,0.2)",
							"rgba(0,0,0,0.3)",
						]} style={styles.videoPostData.dataContainer}>
						{
							!this.props.dontShowArtistName
								?
								<TouchableOpacity
									activeOpacity={1}
									onPress={this.props.onPressArtist}
									style={{
										//  position: 'absolute',
									...
									{/* <Text style={styles.videoPostData.text14600}>Artist</Text> */}
									<View style={{
										flexDirection: "row",
										//alignItems: "center",
										...
										<View style={{
											...
											<Text style={styles.videoPostData.text30800}>{this.props.firstName}</Text>
											<Text style={styles.videoPostData.text30800}>{this.props.lastName}</Text>
										</View>
									</View>
								</TouchableOpacity>
								:
								<TouchableOpacity
									...
									<Text style={styles.videoPostData.text14600}></Text>
									<View style={{
										flexDirection: "row",
										//alignItems: "center",
										marginTop: constants.vh(6)
										...
										<View style={{
											...
											<Text style={styles.videoPostData.text30800}></Text>
											<Text style={styles.videoPostData.text30800}></Text>
										</View>
									</View>
								</TouchableOpacity>
						}
						<View style={styles.videoPostData.secondryDataContainer}>
							<View style={styles.videoPostData.descriptionContainer}>
								<View style={{
									...
									>
										<Text style={styles.videoPostData.text10bold}>{this.props.dayAgo}</Text>
										<View style={{
											marginStart: constants.vw(10)
										}}>
											<Entypo
												onPress={this.props.onThreeDotPress}
									...
									{/* <View>
                    <Text numberOfLines={1} style={styles.videoPostData.text30800}>{this.props.musicHeader}</Text>
                  </View> */}
									<Text numberOfLines={3} style={[styles.videoPostData.text14600]}>{this.props.description}</Text>
								</View>
								<View>
									<View style={{
										flexDirection: "row",
										...
										/>
										<Text style={[styles.videoPostData.text16900, { marginStart: 9 }]}>{this.props.musicTitle}</Text>
									</View>
									...
									>
										...
										<View
											style={{
												width: `${(100 - Math.round((this.state.currentTime / this.state.duration) * 100))}%`,
												height: 4,
												//borderRadius: 20,
												backgroundColor: constants.Colors.rgba_255_255_255_3,
											}}
							...
							<View style={styles.videoPostData.buttonsContainer}>
								<TouchableOpacity
									...
									style={styles.videoPostData.likeButtonContainer}>
									<Image
										source={this.props.isLiked ? constants.Images.Liked : constants.Images.Unliked}
										resizeMode="contain"
									/>
									<Text style={styles.videoPostData.text14600}>{this.props.likeCount}</Text>
								</TouchableOpacity>
								<TouchableOpacity
									...
									style={styles.videoPostData.commentContainer}>
									<FastImage
										source={constants.Images.MessageIcon}
										style={styles.videoPostData.messageImage}
										resizeMode={FastImage.resizeMode.contain}
									/>
									<Text style={styles.videoPostData.text14600}>{this.props.commentCount}</Text>
								</TouchableOpacity>
								<TouchableOpacity style={styles.videoPostData.commentContainer}
									activeOpacity={1}
									hitSlop={{
										top: 5,
										left: 10,
									...
									disabled={this.props.multipleClickShareDisabled ? this.props.multipleClickShareDisabled : false}
								>
									<FastImage
									...
									{/* <Fontisto
										...
										color={constants.Colors.white}
									/> */}
									<Text style={styles.videoPostData.text12600}>Share</Text>
								</TouchableOpacity>
							</View>
						</View>
					</LinearGradient>

					{this.state.showSeker &&
						<View
							style={[styles.seekbar.container]}
							collapsable={false}
							{...this.player.seekPanResponder.panHandlers}
...
const styles = {
	...
	player: StyleSheet.create({
		...
		dayContainer: {
			position: "absolute"
			, bottom: constants.vh(77),
			flexDirection: "row",
			width: "60%",

			alignItems: 'center',
			marginStart: constants.vw(24)
		},
		rightMainContainer: {
			position: "absolute",
			marginTop: constants.vh(120),
			right: constants.vw(30.5),
			alignSelf: "flex-end",
			alignItems: "center"
		},
		rightContainer: {
	...
	videoPostData: StyleSheet.create({
		// dataContainer: {
		//   position: "absolute",
		...
		//     flexDirection: "row",
		//     alignItems: "center",
		//     marginTop: Platform.OS === "ios" ? constants.vh(46) : constants.vh(0),
		//     width:"100%",
