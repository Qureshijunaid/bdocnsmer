...
import { getUserFromLocal } from '../../../actions/auth';
import {
    ...
    handlePaymentLoader, setHomeTab
} from '../../../actions/home';
import {
    setArtistID, getArtistDetail, getConsumerProfile, getArtistDetailHomePage
...
const HomeFeed = (props) => {
    ...
    useEffect(() => {
        props.dispatch(getUserFromLocal());
        let payload = {
            "pageNumber": state.pageNumber
        }
        // props.dispatch(getRecommendedFeed())

        props.dispatch(getTrendingFeed(payload))
    ...
    async function audioSetup() {
        ...
        try {
            Silent.isEnabled().then((value) => {
                console.log("silent value---->", value);
                if (value) {
                    TrackPlayer.setVolume(0)
                    videoMute.current = true
                    setState({
                        ...state,
                        audioMute: true
                    })
    ...
    const handleToggleTopTab = async (feed, recommended, trending) => {
        ...
        if (trending) {
            if (props.home.trendingFeed.length === 0) {
                handleGetTrending()
            }
        ...
        if (feed) {
            handleGetMyFeed()
            // if (props.home.myFeed.length === 0) {
            //     handleGetMyFeed()
            // }
        }
        if (recommended) {
            if (props.home.recommendedFeed.length === 0) {
    ...
    const handleTopTab = (value) => {
        props.dispatch(setHomeTab(value))
        if (value === "feed") {
            TrackPlayer.reset();
    ...
    const handleOnReachedTrending = () => {
        if (props.home.trendingCount > props.home.trendingFeed.length) {
            state.trendingPageNumber = state.trendingPageNumber + 1
            setState({
                ...state
            })
            let payload = {
                "pageNumber": state.trendingPageNumber
            }
            props.dispatch(getTrendingFeed(payload))
        }
    }

    const handleOnReachedRecommended = () => {
        if (props.home.recomendedCount > props.home.recommendedFeed.length) {
            state.recommendedPageNumber = state.recommendedPageNumber + 1
            setState({
                ...state
            })
            let payload = {
                "pageNumber": state.recommendedPageNumber
            }
            props.dispatch(getRecommendedFeed(payload))
        }
    }
    ...
    const showArtistDetail = (data) => {
        ...
        if (state.multipleClickDisabled) {
            ...
            props.dispatch(getArtistDetailHomePage(data, true, null))
        }

    }

    const renderRecommendedFeed = ({ item, index }) => {
        return (
            <View style={{
                marginVertical: constants.vh(10)
            }}>
                <Components.NonSubscribedArtistCard
                    ...
                    coverImage={
                        item.coverImage != null ? { uri: item.coverImage } :
                            constants.Images.ArtistCover
                    }

                    profileImage={
                        item.profileImage != null ? { uri: item.profileImage } :
                            { uri: constants.AppConstant.bandoLogo }}


                    onPress={() => showArtistDetail(item._id)}
                    onPressShare={() => debounce(() => handleOnShareProfile(item))}
                    // onPressShare={() => debounce(handleBounce, 200)}
                    multipleClickDisabled={state.multipleClickDisabled}
    ...
    const renderTrendingFeed = ({ item, index }) => {
        return (
            <View style={{
                marginVertical: constants.vh(10)
            }}>
    ...
    const handleOnShareProfile = (item) => {
        console.log('handleOnShareProfile')
    ...
    const renderEmptyRecommended = ({ }) => {
        return (
            <View style={{
                ...
                <Components.NoPostFoundFeed
                    title="There are no recommended artists for you right now." />
            </View>
        )
    }
    ...
    const renderEmptyTrending = ({ }) => {
        return (
            <View style={{
                ...
                <Components.NoPostFound
                    title="There is no trending feed to show."
                />
            </View>
        )
    }
        ...
        <>
            ...
            <SafeAreaView style={styles.container}>
                <View style={styles.dataContainer}>
                    ...
                    />
                    {props.home.myFeedTotalCount > 0 ?
                        <View style={styles.statsButtonContainer}>
                            <View style={{
                                width: "30%",
                            }}>
                                <Components.PrimaryButton
                                    onPress={() => { handleTopTab("feed") }}
                                    //onPress={() => handleToggleTopTab(true, false, false)}
                                    borderRadius={100}
                                    paddingVertical={constants.vh(7)}
                                    title={"Feed"}
                                    backgroundColor={props.home.feedTab ? constants.Colors.color_FF3062 : constants.Colors.color_2F2F2F}
                                />
                            </View>
                            <View style={{
                                width: "40%",
                                ...
                                <Components.PrimaryButton
                                    onPress={() => { handleTopTab("recommended") }}
                                    //onPress={() => handleToggleTopTab(false, true, false)}
                                    borderRadius={100}
                                    paddingVertical={constants.vh(7)}
                                    title={"Recommended"}
                                    backgroundColor={props.home.recommendedTab ? constants.Colors.color_FF3062 : constants.Colors.color_2F2F2F}
                                />
                            </View>
                            ...
                            <View style={{
                                ...
                                <Components.PrimaryButton
                                    onPress={() => { handleTopTab("trending") }}
                                    //onPress={() => handleToggleTopTab(false, false, true)}
                                    borderRadius={100}
                                    paddingVertical={constants.vh(7)}
                                    title={"Trending"}
                                    backgroundColor={props.home.trendingTab ? constants.Colors.color_FF3062 : constants.Colors.color_2F2F2F}
                                />
                            </View>
                        </View>
                        :
                    ...
                    {props.home.recommendedTab &&
                        //state.recommended &&
                        <View style={{ flex: 1 }}>
                            <FlatList
                                showsVerticalScrollIndicator={false}
                                data={props.home.recommendedFeed}
                                extraData={props.home.recommendedFeed}
                                renderItem={renderRecommendedFeed}
                                ListEmptyComponent={renderEmptyRecommended}
                                onEndReached={handleOnReachedRecommended}
                                onEndReachedThreshold={0.5}
                                refreshing={state.isRefreshing}
                                onRefresh={handleGetRecommended}
                                keyExtractor={(item, index) => index.toString()}
                            />
                        </View>

                    }
                    {props.home.trendingTab &&
                        //state.trending &&
                        <View style={{ flex: 1 }}>
                            <FlatList
                                showsVerticalScrollIndicator={false}
                                data={props.home.trendingFeed}
                                extraData={props.home.trendingFeed}
                                renderItem={renderTrendingFeed}
                                onEndReached={handleOnReachedTrending}
                                onEndReachedThreshold={0.5}
                                ListEmptyComponent={renderEmptyTrending}
                                refreshing={state.isRefreshing}
                                onRefresh={handleGetTrending}
                                keyExtractor={(item, index) => index.toString()}
                            />
                        </View>
                    ...
                    {props.home.feedTab &&
                        ...
                        <View style={{ flex: 1 }}>
                            <FlatList
                                showsVerticalScrollIndicator={false}
                                data={props.home.myFeed}
                                extraData={props.home.myFeed}
